...
type RestaurantQuery struct {
	...
	loadTotal  []func(context.Context, []*Restaurant) error
...
func (rq *RestaurantQuery) Order(o ...restaurant.OrderOption) *RestaurantQuery {
...
func (rq *RestaurantQuery) All(ctx context.Context) ([]*Restaurant, error) {
	...
	return withInterceptors[[]*Restaurant](ctx, rq, qr, rq.inters)
}
...
func (rq *RestaurantQuery) IDs(ctx context.Context) (ids []int, err error) {
...
func (rq *RestaurantQuery) Count(ctx context.Context) (int, error) {
	...
	return withInterceptors[int](ctx, rq, querierCount[*RestaurantQuery](), rq.inters)
}
...
func (rq *RestaurantQuery) Clone() *RestaurantQuery {
	...
	return &RestaurantQuery{
		config:     rq.config,
		ctx:        rq.ctx.Clone(),
		order:      append([]restaurant.OrderOption{}, rq.order...),
...
func (rq *RestaurantQuery) GroupBy(field string, fields ...string) *RestaurantGroupBy {
	...
	grbuild := &RestaurantGroupBy{build: rq}
...
func (rq *RestaurantQuery) Select(fields ...string) *RestaurantSelect {
	...
	sbuild := &RestaurantSelect{RestaurantQuery: rq}
...
func (rq *RestaurantQuery) Aggregate(fns ...AggregateFunc) *RestaurantSelect {
...
func (rq *RestaurantQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Restaurant, error) {
	var (
		nodes       = []*Restaurant{}
		withFKs     = rq.withFKs
		_spec       = rq.querySpec()
	...
	if query := rq.withOwner; query != nil {
		if err := rq.loadOwner(ctx, query, nodes, nil,
			func(n *Restaurant, e *User) { n.Edges.Owner = e }); err != nil {
			return nil, err
		}
	}
...
func (rq *RestaurantQuery) loadOwner(ctx context.Context, query *UserQuery, nodes []*Restaurant, init func(*Restaurant), assign func(*Restaurant, *User)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Restaurant)
	for i := range nodes {
		if nodes[i].user_restaurants == nil {
			continue
		}
		fk := *nodes[i].user_restaurants
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_restaurants" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (rq *RestaurantQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(restaurant.Table, restaurant.Columns, sqlgraph.NewFieldSpec(restaurant.FieldID, field.TypeInt))
	_spec.From = rq.sql
...
func (rgb *RestaurantGroupBy) Scan(ctx context.Context, v any) error {
	ctx = setContextOp(ctx, rgb.build.ctx, ent.OpQueryGroupBy)
	if err := rgb.build.prepareQuery(ctx); err != nil {
		return err
	}
	return scanWithInterceptors[*RestaurantQuery, *RestaurantGroupBy](ctx, rgb.build, rgb, rgb.build.inters, v)
}

func (rgb *RestaurantGroupBy) sqlScan(ctx context.Context, root *RestaurantQuery, v any) error {
...
func (rs *RestaurantSelect) Aggregate(fns ...AggregateFunc) *RestaurantSelect {
	rs.fns = append(rs.fns, fns...)
	return rs
}

// Scan applies the selector query and scans the result into the given value.
func (rs *RestaurantSelect) Scan(ctx context.Context, v any) error {
	ctx = setContextOp(ctx, rs.ctx, ent.OpQuerySelect)
	if err := rs.prepareQuery(ctx); err != nil {
		return err
	}
	return scanWithInterceptors[*RestaurantQuery, *RestaurantSelect](ctx, rs.RestaurantQuery, rs, rs.inters, v)
}

func (rs *RestaurantSelect) sqlScan(ctx context.Context, root *RestaurantQuery, v any) error {
