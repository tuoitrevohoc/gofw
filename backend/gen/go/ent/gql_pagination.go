...
func (c *RestaurantConnection) build(nodes []*Restaurant, pager *restaurantPager, after *Cursor, first *int, before *Cursor, last *int) {
	c.PageInfo.HasNextPage = before != nil
	c.PageInfo.HasPreviousPage = after != nil
	if first != nil && *first+1 == len(nodes) {
		c.PageInfo.HasNextPage = true
	...
	c.Edges = make([]*RestaurantEdge, len(nodes))
	for i := range nodes {
		node := nodeAt(i)
		c.Edges[i] = &RestaurantEdge{
			Node:   node,
			Cursor: pager.toCursor(node),
		}
	}
...
func newRestaurantPager(opts []RestaurantPaginateOption, reverse bool) (*restaurantPager, error) {
	pager := &restaurantPager{reverse: reverse}
	for _, opt := range opts {
		if err := opt(pager); err != nil {
			return nil, err
		}
...
func (p *restaurantPager) applyCursors(query *RestaurantQuery, after, before *Cursor) (*RestaurantQuery, error) {
	direction := p.order.Direction
	if p.reverse {
		direction = direction.Reverse()
	}
	for _, predicate := range entgql.CursorsPredicate(after, before, DefaultRestaurantOrder.Field.column, p.order.Field.column, direction) {
		query = query.Where(predicate)
	}
	return query, nil
}

func (p *restaurantPager) applyOrder(query *RestaurantQuery) *RestaurantQuery {
	direction := p.order.Direction
	if p.reverse {
		direction = direction.Reverse()
	}
	...
	if p.order.Field != DefaultRestaurantOrder.Field {
		query = query.Order(DefaultRestaurantOrder.Field.toTerm(direction.OrderTermOption()))
	}
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(p.order.Field.column)
	}
...
func (p *restaurantPager) orderExpr(query *RestaurantQuery) sql.Querier {
	direction := p.order.Direction
	if p.reverse {
		direction = direction.Reverse()
	}
	...
	return sql.ExprFunc(func(b *sql.Builder) {
		b.Ident(p.order.Field.column).Pad().WriteString(string(direction))
		if p.order.Field != DefaultRestaurantOrder.Field {
			b.Comma().Ident(DefaultRestaurantOrder.Field.column).Pad().WriteString(string(direction))
		}
	})
}
...
func (r *RestaurantQuery) Paginate(
	...
	pager, err := newRestaurantPager(opts, last != nil)
	if err != nil {
		return nil, err
	}
...
type RestaurantOrderField struct {
	// Value extracts the ordering value from the given Restaurant.
	Value    func(*Restaurant) (ent.Value, error)
	column   string // field or computed.
	toTerm   func(...sql.OrderTermOption) restaurant.OrderOption
...
var DefaultRestaurantOrder = &RestaurantOrder{
	...
	Field: &RestaurantOrderField{
		Value: func(r *Restaurant) (ent.Value, error) {
			return r.ID, nil
		},
		column: restaurant.FieldID,
		toTerm: restaurant.ByID,
